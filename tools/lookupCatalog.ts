    ...
    "lookup_catalog",
    {
      ...
      inputSchema: z.object({
        ...
        shops: z
          .array(z.string())
          .optional()
          .describe("Optional array of specific shop GIDs to filter by (e.g. gid://shopify/Shop/123)."),
        ships_to: z
          .object({
      ...
      const markdown = [
        ...
        ...products.flatMap((product: any, index: number) => {
          ...
          const priceText = typeof price.amount === "number"
            ? new Intl.NumberFormat(context.language, { style: "currency", currency: price.currency || context.currency }).format(price.amount / 100)
            : "Price unavailable";
