...
export function registerUpdateCheckout(server: McpServer, env: Env) {
	server.registerTool(
		"update_checkout",
		{
			...
			description:
				"Update line items, buyer details, or fulfillment information on an active checkout session.",
			inputSchema: z.object({
				shop_domain: z.string().describe("Required. The merchant's shop domain."),
				checkout_id: z
					.string()
					.describe("Required. The ID of the checkout session to update."),
				checkout: z
					.object({
						line_items: z
							.array(
						...
						attribution: z
							...
							.optional()
							.describe("UTM tags, referring domain, etc."),
		...
		async (input) => {
			...
			if (ecpUrl) {
				...
				url.searchParams.set(
					"ec_delegate",
					"fulfillment.address_change,payment.instruments_change,payment.credential",
				);
				ecpUrl = url.toString();
			}

			const totalAmount =
				checkoutData?.totals?.find((t: any) => t.type === "total")?.amount || 0;
			...
			const markdown = [
				...
				...messages.map(
					(msg: any) =>
						`⚠️ ${msg.severity ? msg.severity.toUpperCase() : "NOTE"}: ${msg.content || msg.message}`,
				),
				"",
				"AGENT DIRECTIVE:",
				checkoutData?.status === "incomplete"
					? "The checkout is incomplete. Prompt the user directly for any missing information."
					: "",
				checkoutData?.status === "requires_escalation"
					? "Pass the Embedded Checkout URL to the frontend UI so the buyer can securely enter shipping/payment details."
					: "",
				checkoutData?.status === "ready_for_complete"
					? "The cart is locked and loaded. Run `complete_checkout` immediately."
					: "",
			]
				.filter(Boolean)
				.join("\n");
