...
export function registerCompleteCheckout(server: McpServer, env: Env) {
	server.registerTool(
		"complete_checkout",
		{
			...
			description:
				"Submit payment and finalize the order. Use ONLY when checkout status is ready_for_complete. Requires passing payment instruments if handled natively.",
			inputSchema: z.object({
				shop_domain: z.string().describe("Required. The merchant's shop domain."),
				checkout_id: z
					.string()
					.describe("Required. The ID of the checkout session to complete."),
				checkout: z
					.object({
						payment: z
							.object({
					...
					.optional()
					.describe("Optional checkout payload containing payment credentials."),
			}),
		},
		async (input) => {
