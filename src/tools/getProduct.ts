...
export function registerGetProduct(server: McpServer, env: Env) {
	server.registerTool(
		"get_product", // FIXED: Removed trailing space
		{
			...
			description:
				"Retrieves full details for a single product, including available options (sizes, colors) and specific variants. Use when a customer wants more info on a specific item or needs to select options.",
			inputSchema: z.object({
				id: z
					.string()
					.describe(
						"Required. Product or variant identifier. Accepts gid://shopify/p/{upid} or gid://shopify/ProductVariant/{id}.",
					),
				selected: z
					.array(
						z.object({
					...
					.optional()
					.describe(
						"Option selections for variant narrowing. Example: [{'name': 'Color', 'label': 'Black'}]",
					),
				preferences: z
					.array(z.string())
					.optional()
					.describe(
						"Option names in relaxation priority order. Example: ['Color', 'Size'].",
					),
				in_stock: z
					.boolean()
					.optional()
		...
		async (input) => {
			...
			if (product) {
				...
				const description =
					...
					product.description?.html?.replace(/<[^>]*>?/gm, "") ||
					"";
				...
				if (metadata.top_features?.length || metadata.unique_selling_points?.length) {
					...
					if (metadata.unique_selling_points?.length) {
						markdown.push(
							`**Why it stands out:** ${metadata.unique_selling_points.join(" • ")}`,
						);
					}
					markdown.push("");
				}
				...
				markdown.push(product.url ? `[View on store](${product.url})` : "", "---");
				...
				if (product.variants && product.variants.length > 0) {
					...
					product.variants.forEach((variant: any) => {
						...
						const promotedText = placement
							? "⚠️ Sponsored / Promoted placement — we may earn a commission."
							: "";
