    ...
    async (input) => {
      ...
      const upstream = await fetch("https://catalog.shopify.com/api/ucp/mcp", {
        ...
        body: JSON.stringify({
          ...
          method: "tools/call",
          id: 3,
          params: { name: "get_product", arguments: { meta: { "ucp-agent": { profile: env.AGENT_PROFILE_URL } }, catalog } }
        })
      });
        ...
        if (product.variants && product.variants.length > 0) {
          ...
          product.variants.forEach((variant: any) => {
            ...
            markdown.push(
              ...
              variant.checkout_url ? `[Buy Now](${variant.checkout_url})` : "", // Checkout link left exactly as Shopify provided
              `ID: \`${variant.id}\``,
              ""
            );
          });
      ...
      return { content: [{ type: "text" as const, text: markdown.filter(Boolean).join("\n") }] };
    }
  );
}