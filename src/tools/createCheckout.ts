...
export function registerCreateCheckout(server: McpServer, env: Env) {
	server.registerTool(
		"create_checkout",
		{
			...
			inputSchema: z.object({
				shop_domain: z
					.string()
					.describe(
						"Required. The shop domain where the checkout is happening (e.g., 'example.myshopify.com').",
					),
				cart_id: z
					.string()
					.optional()
				...
				checkout: z
					.object({
						...
						line_items: z
							...
							.optional()
							.describe(
								"Required if cart_id is not provided. Array of items to purchase.",
							),
						buyer: z
							.object({
							...
							.optional()
							.describe(
								"Required if cart_id is not provided. Must include at least email or phone_number.",
							),
						fulfillment: z
							.object({
								methods: z
		...
		async (input) => {
			...
			if (!checkoutData) {
				return {
					content: [
						{
							type: "text" as const,
							text: "Failed to create checkout. No structured content returned from the server.",
						},
					],
				};
			}
			...
			const totalDisplay =
				checkoutData.totals?.find((t: any) => t.type === "total")?.display_text || "Total";
