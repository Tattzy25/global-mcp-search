		...
		"search_products",
		{
			...
			inputSchema: z.object({
				...
				like: z
						...
						z.union([
							z.object({
								id: z
									.string()
									.describe(
										"Product or variant GID, e.g., gid://shopify/Product/123 or gid://shopify/ProductVariant/123",
									),
							}),
							z.object({
								image: z.object({
									content_type: z
										.string()
										.describe(
											"Image MIME type, e.g., image/jpeg, image/png, or image/webp.",
										),
									data: z
										.string()
										.describe(
											"Base64-encoded image data for visual similarity search.",
										),
								}),
							}),
						]),
					...
					.optional()
					.describe(
						"Similar item(s) to search by. Use for visual similarity or multimodal search. Pass an array of objects containing either 'id' or 'image'.",
					),
				...
				image: z
					.object({
						content_type: z
							.string()
							.describe(
								"Image MIME type, for example image/jpeg, image/png, or image/webp.",
							),
						data: z
							.string()
							.describe(
								"Base64-encoded image data for visual or multimodal similarity search.",
							),
					})
					.optional()
					.describe(
						"Optional image for similarity search. Combine with search to describe what should match the image. Do not use for a URL lookup.",
					),
				...
				ships_to: z
					...
					.optional()
					.describe(
						"Optional delivery destination used to return products that ship to the buyer.",
					),
				...
				currency: z.string().optional().describe("Buyer currency code, for example USD."),
